...
def handle_find_element():
    ...
    if symbol:
        if group:
            messagebox.showinfo('Element Found', f"{symbol} is at group {group} on the periodic table and period {period}.")
        elif name:
            symbol = find_element_name(symbol)
            group = find_element_group(symbol)
            period = find_element_period(symbol)
            messagebox.showinfo('Element Found', f"{symbol} is at group {group} on the periodic table and period {period}.")
        else:
        ...
        messagebox.showwarning('Empty Input', 'Please enter an element symbol.')
